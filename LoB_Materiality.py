...
def section_2_investment_activities(session_state):
    ...
    country = gb_cols[1].selectbox(f"country_{i}", options=countries, key=f"country_{i}", help=f"Select the country for government bond {i + 1}")
    exposure = gb_cols[2].selectbox(f"exposure_{i}", options=["Low", "Medium", "High", "Not relevant/No Exposure"], key=f"exposure_gb_{i}", help=f"Select the exposure level for government bond {i + 1}")
